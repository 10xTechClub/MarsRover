...
Servo scanServo, hallScannerServo, servoPan, servoTilt;
...
void showAllLEDs();
void smoothServoMove(Servo &servo, int target, int stepDelay);
...
void sendFullData() {
  ...
  // If no alerts were triggered, send "All clear"
  if (alerts.size() == 0) {
    alerts.add("✅ All sensors within safe range");
  }
...
void handleCommand(const String &msg) {
  ...
  if (msg.startsWith("mode:")) {
    ...
    else if (msg.startsWith("hallscanner:")) {
      int target = constrain(msg.substring(12).toInt(), 0, 180);
      smoothServoMove(hallScannerServo, target, 50); // 50 ms per step
    }
  } else {
...
void initializeSystem() {
  ...
  hallScannerServo.attach(HALL_SERVO);
  servoPan.attach(SERVO_PAN_PIN);
  servoTilt.attach(SERVO_TILT_PIN);

  scanServo.write(90);
  hallScannerServo.write(90);
  servoPan.write(90);
  servoTilt.write(90);
